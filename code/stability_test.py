    ...
    for ratio in np.linspace(0.0001736, 0.0006944, 15):
        ...
        for end_time in range(int(history_start_time_), int(history_end_time), one_day * slide_step * 1000):
            ...
            if (s_tf[4]):
                drop += 1
                for failure_segment in failure_segments:
                    if (timeseries_config["start_time"] > failure_segment[0] and timeseries_config["start_time"] < failure_segment[1]) or (timeseries_config["end_time"] > failure_segment[0] and timeseries_config["end_time"] < failure_segment[1]) or (
                    timeseries_config["start_time"] < failure_segment[0] and timeseries_config["end_time"] >
                    failure_segment[1]):
                        right += 1
        ...
        failure_metric_x.append(ratio)
        failure_metric_y.append(drop/tot)
        failure_metrci_y1.append(right/expect_right_time)

    print(failure_metric_x)
    print(failure_metric_y)
    print(failure_metrci_y1)
